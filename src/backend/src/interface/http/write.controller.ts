...
import { asS7Area, asS7Type } from '../../driver/siemens/s7.types';
...
export class WriteController {
  ...
  async write(@Body() body: { tagId: number; value: any }) {
    const tag = await this.prisma.tag.findUnique({ where: { id: body.tagId }, include: { plc: true } });
    ...
    await this.s7.writeOne(
      c,
      {
        area: asS7Area(tag.area),                 // <-- narrow from string to union
        dbNumber: tag.dbNumber ?? undefined,
        start: tag.start,
        amount: tag.amount,
        dataType: asS7Type(tag.dataType),         // <-- narrow from string to union
      },
      body.value,
    );
